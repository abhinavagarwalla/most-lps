...
import pickle
from det3d.datasets import semantickitti
...
from det3d.datasets.semantickitti import semantickitti_common as semantickitti_ds
...
def semantic_kitti_data_prep(root_path, split, nsweeps=1):
    # semantickitti_ds.create_semantickitti_infos(root_path, split=split, nsweeps=nsweeps)
    if split != 'train':
        create_groundtruth_database(
            "SemanticKITTI",
            root_path,
            Path(root_path) / "infos_train_single_sweep.pkl"
        )
