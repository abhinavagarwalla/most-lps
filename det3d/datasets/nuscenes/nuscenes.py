...
class NuScenesDataset(PointCloudDataset):
        ...
        **kwargs,
        ...
        if class_names is not None:
            super(NuScenesDataset, self).__init__(
                root_path, info_path, pipeline, test_mode=test_mode, class_names=class_names[:10]
            )
        else:
            super(NuScenesDataset, self).__init__(
        ...
        if class_names is not None:
            self._class_names = class_names[:10] #load infos only for objects
            self._sem_class_names = class_names[:16]
        else:
    ...
    def evaluation_panoptic(self, predictions, output_dir=None, testset=False):
        if testset:
            print(self.version)
            assert(self.version == "v1.0-test")
        ...
        if not testset:
            ...
            for gt in gt_annos:
                try:
                    preds.append(predictions[gt["token"]])
                except Exception:
                    miss += 1
            ...
            assert len(predictions) == 6008

        nusc = NuScenes(version=version, dataroot=str(self._root_path), verbose=True)

        mapped_class_names = []
        for n in self._sem_class_names:
            if n in self._name_mapping:
                mapped_class_names.append(self._name_mapping[n])
            else:
                mapped_class_names.append(n)

        save_pred_dir = str(Path(output_dir) / Path('panoptic/'+eval_set_map[self.version]))
        os.makedirs(save_pred_dir, exist_ok=True)
        for pred in preds:
            token = nusc.get('sample', pred["metadata"]['token'])['data']['LIDAR_TOP']
            npz_file_out = os.path.join(save_pred_dir, token + '_panoptic.npz')
            
            nuscenes_pano_preds = 1000*pred['sem_preds_pt'].numpy() #assigning instance=0 to all since only evaluation lidarseg
            np.savez_compressed(npz_file_out, data=nuscenes_pano_preds.astype(np.uint16))
        ...
        if not testset:
            ...
            with open(Path(output_dir) / "segmentation-result.json", "r") as f:
                metrics = json.load(f)

            detail = {}
            result = f"Nusc {version} Evaluation\n"
            for name in mapped_class_names:
                iou = metrics["segmentation"][name]["IoU"]
                pq = metrics["segmentation"][name]["PQ"]

                result += f"{name} Nusc IoU: {iou*100:.2f}, "
                result += f"PQ: {pq*100:.2f}\n"

                detail[name] = {"miou": iou, "pq": pq}
            res_nusc = {
                "results": {"nusc": result},
                "detail": {"nusc": detail},
            }
