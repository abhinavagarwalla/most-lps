...
def create_nuscenes_infos(root_path, version="v1.0-trainval", nsweeps=10, filter_zero=True):
    ...
    if test:
        ...
        # root_path = Path("/project_data/ramanan/aa4/datasets/nuscenes/")
        with open(
...
def eval_pano_main(nusc, eval_version, res_folder, eval_set, output_dir, min_inst_points=15):
    # nusc = NuScenes(version=version, dataroot=str(root_path), verbose=True)
    
    nusc_eval = NuScenesPanopticEval(
        nusc,
        results_folder=res_folder,
        eval_set=eval_set,
        task='segmentation',
        min_inst_points=min_inst_points,
        out_dir=output_dir,
        verbose=True,
    )
    nusc_eval.evaluate()
    print(f'Evaluation results saved at {output_dir}/segmentation-result.json. \nFinished segmentation evaluation.')
