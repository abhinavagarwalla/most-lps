...
#Modified from : Chao Zhe Cylinder3D Github Repo
def geo_loss(voxel_label, voxel_label_pre, ignore_label=5):
    # start = time.time()

    voxel_shape = voxel_label.shape
    ratio = torch.nn.functional.log_softmax(voxel_label_pre, dim=1)
    voxel_label_pre = torch.argmax(voxel_label_pre.detach(), dim=1)
    full_voxle_index = torch.stack(torch.where(voxel_label!=ignore_label), dim=1)
    neighbor_num = 8
    
    neighbor_list = []
    zero_mat = torch.zeros_like(full_voxle_index)
    ...
    # Weight for a voxel
    M_LGA = torch.zeros((full_voxle_index.shape[0],)).cuda()
    for i, neighbor in enumerate(neighbor_list):

        # clamp to range
        neighbor[:, 1] = torch.clamp(neighbor[:, 1], min=0, max=voxel_shape[1]-1)
        neighbor[:, 2] = torch.clamp(neighbor[:, 2], min=0, max=voxel_shape[2]-1)
        neighbor[:, 3] = torch.clamp(neighbor[:, 3], min=0, max=voxel_shape[3]-1)

        mask = (voxel_label_pre[neighbor[:, 0], neighbor[:, 1], neighbor[:, 2], neighbor[:, 3]] != \
                voxel_label[neighbor[:, 0], neighbor[:, 1], neighbor[:, 2], neighbor[:, 3]]).to(torch.int)
    ...
    w = torch.zeros(voxel_shape).cuda()
