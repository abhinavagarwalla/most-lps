...
from det3d.models.losses.centernet_loss import FastFocalLoss, RegLoss, SemanticSegmentationLoss
...
class CenterHead(nn.Module):
    def __init__(
        ...
        self.sem_crit = SemanticSegmentationLoss()

        self.box_n_dim = 3
        if 'vel' in common_heads:
            self.box_n_dim += 2
        if 'dim' in common_heads:
            self.box_n_dim += 3
        ...
        for cls_i, num_cls in enumerate(num_classes):
            heads = copy.deepcopy(common_heads)
            if not dcn_head:
                if cls_i >= 6: #dont add other targets for segmentation
                    heads = dict(hm=(num_cls, num_hm_conv))
                else:
    ...
    def visualize_pred_bev(self, pred_heatmaps, gt_heatmaps, version=2):
        from matplotlib import pyplot as plt
        fig, ax = plt.subplots(nrows=2, ncols=pred_heatmaps.shape[0])

        ax[0][0].set_ylabel('Predicted')
        ax[1][0].set_ylabel('GT')
        class_names = {11: 'driveable_surface', 12: 'other_flat', 13: 'sidewalk', 14: 'terrain', 15: 'manmade', 16: 'vegetation'}


        if version == 1:
            for i in range(pred_heatmaps.shape[0]):
                ax[0][i].imshow(pred_heatmaps[i])
                ax[1][i].imshow(gt_heatmaps[i])
                ax[0][i].set_title(f'Class: {class_names[i+11]}')
            ...
            pred_class = pred_heatmaps.argmax(axis=0)
            pred_heatmaps = [(pred_class==i).astype(int) for i in range(pred_heatmaps.shape[0])]
    ...
    def loss(self, example, preds_dicts, test_cfg, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            # heatmap loss
            if task_id >= 6: #specific to nuscenes.
                # no localization loss for segmentation
                # preds_dict['hm'] = torch.softmax(preds_dict['hm'], dim=1)
                hm_loss = self.sem_crit(preds_dict['hm'], example['hm'][task_id], example['ind'][task_id], example['mask'][task_id], example['cat'][task_id])

                loss = hm_loss
                ret = {}
                ret.update({'loss': loss, 'hm_loss': hm_loss.detach().cpu(),
                            'loc_loss': torch.tensor(0, device=loss.device),
                            'loc_loss_elem': torch.tensor([0]*self.box_n_dim),
                ...
                if self.dataset in ['waymo', 'nuscenes', 'semantickitti']:
                    if 'vel' in preds_dict:
                        if 'dim' in preds_dict:
                            preds_dict['anno_box'] = torch.cat((preds_dict['reg'], preds_dict['height'],
                                                                preds_dict['dim'], preds_dict['vel']), dim=1)
                        else:
                            preds_dict['anno_box'] = torch.cat((preds_dict['reg'], preds_dict['height'],
                                                                preds_dict['vel']), dim=1)
                            target_box = target_box[..., [0, 1, 2, 6, 7]] # remove dimensions target
                    else:
                        if 'dim' in preds_dict:
                            preds_dict['anno_box'] = torch.cat((preds_dict['reg'], preds_dict['height'],
                                                                preds_dict['dim'], preds_dict['vel']), dim=1)
                            target_box = target_box[..., [0, 1, 2, 3, 4, 5]]
                        else:
                            preds_dict['anno_box'] = torch.cat((preds_dict['reg'], preds_dict['height']), dim=1)   
                            target_box = target_box[..., [0, 1, 2]] # remove vel target                       
    ...
    def predict(self, example, preds_dicts, test_cfg, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            if task_id >= 6: #segmentation task
                assert(not double_flip)
                sem_rets.append(batch_hm)# > test_cfg.score_threshold)
                continue
            ...
            batch_rot = torch.zeros_like(batch_hei)
            ...
            if 'vel' in preds_dict:
                ...
                if 'dim' in preds_dict:
                    ...
                    batch_box_preds = torch.cat([xs, ys, batch_hei, batch_vel, batch_rot], dim=2)
            else:
                if 'dim' in preds_dict:
                    batch_box_preds = torch.cat([xs, ys, batch_hei, batch_dim, batch_rot], dim=2)
                else:
                    batch_box_preds = torch.cat([xs, ys, batch_hei, batch_rot], dim=2)
        ...
        # add in semantic predictions
        try:
            sem_preds = torch.cat(sem_rets, dim=-1)
            for i in range(num_samples):
                ret_list[i]["sem_preds"] = sem_preds[i]
        except:
            print("No seg head")
